...
import { Settings, Smartphone, Sparkles, RefreshCw, Percent, FileText, Database, Copy, Check } from 'lucide-react';
      ...
      <div className="space-y-8 flex-1 overflow-y-auto pr-2">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <input 
              ...
              className="w-full bg-black/40 border border-white/5 text-white px-4 py-3 rounded-xl focus:ring-1 focus:ring-indigo-500/50 focus:border-indigo-500/50 outline-none transition-all placeholder:text-neutral-700 text-sm" 
            />
          </div>
          <div className="space-y-2">
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
              {['⚡', '🔥', '💎', '🍔', '🍕', '🍹', '🎁'].map(emoji => (
                  <button 
                    key={emoji} 
                    onClick={() => setConfig({...config, logoEmoji: emoji})} 
                    className={cn(
                      "w-10 h-10 rounded-lg border flex items-center justify-center text-lg transition-all shrink-0",
                      config.logoEmoji === emoji 
                        ? "bg-indigo-600 border-indigo-500 text-white shadow-[0_0_15px_-3px_rgba(99,102,241,0.4)]" 
          ...
          <div className="grid grid-cols-1 gap-4">
            <div className="space-y-2">
              <span className="text-xs text-neutral-400 ml-1 flex items-center gap-1">
                Premio Cebo <span className="text-neutral-600">(Fácil)</span>
              </span>
              <input 
                type="text" 
                name="prizeTextSmall" 
                value={config.prizeTextSmall} 
                onChange={handleChange} 
                className="w-full bg-black/40 border border-white/5 text-white px-4 py-3 rounded-xl focus:border-yellow-500/50 outline-none transition-all placeholder:text-neutral-700 text-sm" 
              />
            </div>
            <div className="space-y-2">
              <span className="text-xs text-neutral-400 ml-1 flex items-center gap-1">
                Premio Jackpot <Sparkles size={10} className="text-purple-400"/>
              </span>
              <input 
                type="text" 
                name="prizeTextBig" 
                value={config.prizeTextBig} 
                onChange={handleChange} 
                className="w-full bg-black/40 border border-white/5 text-white px-4 py-3 rounded-xl focus:border-purple-500/50 outline-none transition-all placeholder:text-neutral-700 text-sm shadow-[0_0_20px_-5px_rgba(168,85,247,0.1)]" 
              />
            </div>
          </div>
        </div>
        ...
        <div className="p-5 bg-gradient-to-b from-white/5 to-transparent rounded-2xl border border-white/5 space-y-4">
           <div className="flex justify-between items-center">
              <label className="text-[10px] font-bold text-indigo-300 uppercase tracking-widest flex items-center gap-2">
                <Percent size={12}/> Probabilidad de Ganar
              </label>
              <span className="text-xs font-mono text-white bg-indigo-500/20 px-2 py-1 rounded">
           ...
           />
           <div className="flex justify-between text-[10px] text-neutral-500 font-medium uppercase">
             <span>Imposible</span>
             <span>Justo</span>
             <span>Regalo</span>
           </div>
        ...
        <div className="space-y-2">
            <span className="text-xs text-neutral-400 ml-1 flex items-center gap-1">
              <FileText size={12}/> Términos y Condiciones (Letra Chica)
            </span>
            <textarea 
              name="legalText" 
      ...
      <div className="mt-6 pt-4 border-t border-white/5 space-y-3">
        ...
        >
          {copied ? (
            ...
            <>
              <Copy size={16} /> Copiar Link para Instagram
            </>
          )}
        </button>
        ...
        <div className="flex items-center justify-between">
            ...
            <button 
              onClick={() => { 
                localStorage.removeItem(`advergame_${config.brandName}`); 
