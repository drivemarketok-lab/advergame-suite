...
const SlotMachine = ({ config }) => {
  ...
  const [acceptedTerms, setAcceptedTerms] = useState(false); // <--- RECUPERADO
      ...
      <div className="w-full space-y-3" onClick={unlockAudio} onTouchStart={unlockAudio}>
         ...
         {(gameState === 'idle' || gameState === 'ready') && (
           <button 
             ...
             className={cn(
               "w-full py-4 rounded-xl font-black text-sm tracking-[0.2em] uppercase bg-gradient-to-b from-indigo-500 to-indigo-700 text-white shadow-lg",
               gameState === 'idle' && (!isValidEmail || !acceptedTerms) ? "opacity-50 cursor-not-allowed" : ""
             )}
           >
