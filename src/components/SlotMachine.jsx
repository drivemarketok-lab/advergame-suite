  ...
  const SlotMachine = ({ config }) => {
    ...
    const playSound = (type) => {
      ...
      const audioObj = audioRefs.current[type];
    ...
    const handleUnlockGame = () => {
      ...
      if (config.webhookUrl) {
          ...
          }).catch(err => console.log("Error envío:", err));
      }
             ...
             <div className="bg-[#1a1a1a] border border-yellow-500/20 p-5 rounded-2xl text-center relative group">
               <div className="relative z-10">
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-4">
                    ...
                    {/* 🔧 FIX: unlockAudio agregado */}
                    <button 
                      onClick={() => { unlockAudio(); handleSpin(true); }} 
                      className="py-3 bg-yellow-500 text-black rounded-xl text-xs font-bold"
                    >
                      ARRIESGAR
                    </button>
      ...
      <div className="w-full max-w-[400px] mx-auto bg-[#050505] rounded-[3rem] shadow-2xl overflow-hidden min-h-[750px] flex flex-col border border-white/10">
          <div className="bg-black/40 p-6 flex justify-between items-center border-b border-white/5">
             ...
             <div className="flex items-center gap-3">
                 ...
                 <button onClick={() => setSoundEnabled(!soundEnabled)} className="text-neutral-400 hover:text-white transition-colors">
          ...
          <div className="bg-black py-4 px-6 text-center border-t border-white/5">
            <p className="text-[9px] text-neutral-600 font-medium leading-normal">
              {config.legalText}
            </p>
            <p className="text-[8px] text-neutral-800 font-bold uppercase tracking-widest mt-2">
              Security by AdverGame Suite™
            </p>
          </div>
      </div>
    );
